...
static int alias_certificate_info(HSM_CLIENT_HANDLE hsm_handle)
{
    ...
    if ((certificate = hsm_client_riot_get_alias_key(hsm_handle)) == NULL)
    {
        (void)printf("Failure getting private key\r\n");
        result = __LINE__;
    }
    else
    {
        (void)printf("Device Private Key:\r\n%s\r\n", certificate);
        free(certificate);
        result = 0;
    }
    return result;
...
static int ca_root_certificate_info(HSM_CLIENT_HANDLE hsm_handle)
{
    ...
    if (result == 0)
    {
        ...
        if ((root_cert = hsm_client_riot_get_root_key(hsm_handle)) == NULL)
        {
            (void)printf("Failure getting root key\r\n");
            result = __LINE__;
        }
    }
    if (result == 0)
    {
        (void)printf("root key:\r\n%s\r\n", root_cert);
        free(root_cert);
